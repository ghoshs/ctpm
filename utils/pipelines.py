...
def prompt__all_criteria_koopman(patient: Dict[str, Any], 
    ...
    """Given a clinical note and all criteria, constructs a prompt for the LLM."""
    # SyntaxError: "f-string expression part cannot include a backslash", replace '\n' with chr(10)
    inclusion_criteria: str = "\n".join([ f"- inclusion_criteria_{i}: {criteria.replace(chr(10), '; ')}" for i,criteria in enumerate(trail["inclusion_criteria"]) ])
    exclusion_criteria: str = "\n".join([ f"- exclusion_criteria_{i}: {criteria.replace(chr(10), '; ')}" for i,criteria in enumerate(trail["exclusion_criteria"]) ])
